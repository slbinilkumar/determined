...
func Initialize(
	...
	// We use the agents endpoint for the default resource provider.
...
func (p *pods) startEventListener(ctx *actor.Context) {
	...
	ctx.Tell(p.eventListener, startEventListener{})
}
...
func (p *pods) summarize(ctx *actor.Context) map[string]agent.AgentSummary {
	...
	for _, result := range results {
		podByNode[result.(podNodeInfo).podName] = append(
			podByNode[result.(podNodeInfo).podName], result.(podNodeInfo))
	}
